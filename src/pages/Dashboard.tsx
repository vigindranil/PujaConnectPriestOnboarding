              ...
              <div className="hidden sm:flex items-center gap-1 px-2 border-l border-r border-stone-100 h-5">
                {(() => {
                  ...
                  if (end - start < 2 && totalPages > 2) {
                    if (start === 1) end = Math.min(3, totalPages - 1);
                    else if (end === totalPages - 1) start = Math.max(1, totalPages - 3);
                  }

                  if (start > 1) {
                    pages.push(
                      <button key={1} onClick={() => handlePageJump(1)} className="w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold text-stone-500 hover:bg-stone-100 transition-colors">1</button>
                    );
