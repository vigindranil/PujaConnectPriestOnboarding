...
export const Dashboard: React.FC = () => {
  ...
  const [selectedPriest, setSelectedPriest] = useState<PriestDetails | null>(
    null,
  );
  ...
  const loadPriestData = useCallback(
    async (pageNo: number = 0) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const handleAddPriest = (p: any) => {
    setPriests([
      ...priests,
      { ...p, id: Date.now(), agentId: userData?.user_id || "A1" },
    ]);
  ...
  const filtered = allpriests.filter((p) => {
    ...
    return filterStatus === "all"
      ? match
      : match && (p.status || "").toLowerCase() === filterStatus;
  })
    ...
    <div className="min-h-screen relative w-full">
      ...
      <div className="fixed -top-40 -right-40 w-[800px] h-[800px] pointer-events-none -z-30">
        ...
        <div className="absolute inset-0 text-rose-200/25 animate-spin-reverse">
          ...
          >
            <path
              d="M100 10 L115 75 L180 90 L115 105 L100 170 L85 105 L20 90 L85 75 Z"
              strokeWidth="1.5"
            />
            <circle cx="100" cy="100" r="50" strokeWidth="1" />
            <circle cx="100" cy="100" r="70" strokeWidth="0.5" />
            <polygon
              points="70,100 100,70 130,100 100,130"
              strokeWidth="1"
      ...
      <div className="fixed bottom-0 left-0 w-[600px] h-[400px] opacity-20 pointer-events-none -z-30 animate-float">
        <svg viewBox="0 0 300 200" className="w-full h-full text-red-800">
          <path
            d="M0 200 L150 50 L300 200 Z"
            fill="none"
            stroke="currentColor"
            strokeWidth="1"
          />
          <path
            d="M50 200 L150 100 L250 200 Z"
            fill="none"
            stroke="currentColor"
            strokeWidth="0.5"
          />
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6 relative">
          <div>
            ...
            <h1 className="text-5xl flex md:text-6xl text-transparent bg-clip-text bg-gradient-to-r from-stone-800 to-stone-600 drop-shadow-sm">
              <div className="mt-4 font-sacred font-bold  ">
                Pranam, {userData?.user_full_name?.split(" ")[0] || "Agent"}
              </div>

              <div className="">
                {" "}
                <img src={flameIcon} alt="flame" className=" w-20 h-20" />
              </div>
            </h1>
            <p className="text-stone-500 font-medium text-lg mt-2 max-w-xl">
              Manage your spiritual network. Monitor registrations and activity
              in real-time.
            </p>
          </div>
        ...
        <div className="rounded-2xl overflow-hidden backdrop-blur-sm bg-white/80 border border-white/50 shadow-[8px_8px_16px_#E6DCC8] mb-6">
          <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="group w-full md:w-[480px] flex items-center bg-stone-50 rounded-xl p-1.5 stone-inset focus-within:ring-2 focus-within:ring-orange-100 transition-all">
              ...
              <button className="flex-shrink-0 relative overflow-hidden bg-gradient-to-r from-orange-600 to-red-600 text-white px-6 py-2.5 rounded-lg font-bold text-xs tracking-widest uppercase shadow-md hover:shadow-lg transition-all hover:-translate-y-0.5">
      ...
      <div className="stone-card rounded-2xl overflow-visible backdrop-blur-sm bg-white/80 border border-white/50">
            ...
            <table className="w-full">
              <thead>
                <tr className="text-xs uppercase tracking-widest font-bold">
                  <th className="p-6 pl-8 font-sacred text-sm text-orange-900 text-left">
                    Priest Identity
                  </th>
                  <th className="p-6 text-amber-900 text-left">Contact Info</th>
                  <th className="p-6 text-orange-800 text-left">
                    Date Of Birth
                  </th>
                  ...
                  <th className="p-6 text-right pr-8 text-amber-900">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((priest, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        className="table-row-animate group bg-white hover:bg-orange-50/20 relative hover:z-50"
                      ...
                      >
                        <td className="p-3 pl-6 w-[16%]">
                          <div className="flex items-center gap-2">
                            <div className="w-9 h-9 flex-shrink-0 rounded-lg bg-gradient-to-br from-orange-100 to-amber-200 text-orange-800 flex items-center justify-center font-sacred text-base font-bold shadow-inner border border-orange-200">
                              {priest.priest_name?.charAt(0).toUpperCase() ||
                                "P"}
                            </div>
                        ...
                        <td className="p-3 w-[15%]">
                          <div className="flex flex-col gap-0.5">
                            <span className="font-mono text-stone-700 font-semibold text-xs">
                              {priest.priest_contact_no}
                            </span>
                            <span className="text-xs text-stone-400 truncate">
                              {priest.priest_email || "N/A"}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="p-3 w-[10%]">
                          <span className="text-xs text-stone-700 font-medium whitespace-nowrap">
                            {priest.priest_dob || "N/A"}
                          </span>
                        </td>

                        <td className="p-3 w-[8%]">
                          <span className="text-xs text-stone-700 font-medium">
                            {priest.priest_gender}
                          </span>
                        </td>

                        <td className="p-3 w-[12%]">
                          <span className="text-xs text-stone-700 font-medium truncate block">
                            {priest.priest_present_city_town_village}
                          </span>
                        </td>
                        ...
                        <td className="p-3 w-[9%]">
                          <span className="text-stone-600 text-xs font-bold uppercase">
                            {priest.priest_experience_year || 0} Yrs
                          </span>
                        </td>
                          ...
                          <ActionDropdown
                            onView={() => {
                              setSelectedPriest({
                                ...
                                priest_present_city_town_village:
                                  priest.priest_present_city_town_village,
                                priest_present_post_office:
                                  priest.priest_present_post_office,
                                priest_experience_year:
                                  priest.priest_experience_year,
                                status: status,
                              });
                            ...
                            onDelete={() => {
                              if (confirm("Delete this priest?"))
                                handleDeletePriest(priest.priest_user_id);
                            }}
                          />
                        </td>
                      </tr>
                  ...
                  <tr>
                    <td colSpan={9} className="py-16 text-center">
                      ...
                      <h3 className="text-lg font-sacred font-bold text-stone-600">
                        No priests found
                      </h3>
          ...
          <div className="border-t border-stone-200 bg-stone-50/50 p-4 sm:px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-stone-400 uppercase tracking-widest">
                Showing {Math.min(currentPage * pageSize + 1, filtered.length)} - {Math.min((currentPage + 1) * pageSize, filtered.length)} of {filtered.length} Records
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 bg-white p-1.5 rounded-full border border-stone-200 shadow-sm">
              ...
              <div className="hidden sm:flex items-center gap-1 px-2 border-l border-r border-stone-100 h-5">
                {(() => {
                  // const totalCount =
                  //   totalRecords ||
                  //   (filtered.length > pageSize ? filtered.length : 50);
                  ...
                  if (start > 0) {
                    pages.push(
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold text-stone-500 hover:bg-stone-100 transition-colors"
                      >
                        1
                      </button>,
                    );
      ...
      {showViewModal && selectedPriest && (
        <ViewEditPriestModal
          ...
          onDelete={() => {
            if (confirm("Delete?"))
              handleDeletePriest(selectedPriest.priest_user_id);
          }}
        />
      )}
    </div>
...
const StatCard3D = ({ label, value, icon, color, trend, delay }: any) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute -right-10 -top-10 w-32 h-32 bg-gradient-to-br ${colorConfig.iconBg} to-transparent rounded-full opacity-30 pointer-events-none group-hover:scale-110 transition-transform duration-500`}
      ></div>
      <div
        className={`absolute right-4 top-4 w-12 h-12 rounded-full opacity-10 ${color.replace("text-", "bg-")} blur-xl group-hover:opacity-25 transition-opacity duration-500`}
      ></div>

      {/* Header Section (Icon + Label) */}
      <div className="flex justify-between items-start relative z-10">
        <p className="text-stone-500 text-xs font-bold uppercase tracking-[0.2em] pt-1">
          {label}
        </p>
      ...
      <div className="relative z-10">
        <h3
          className={`text-5xl font-mono font-bold text-stone-800 tracking-tighter ${colorConfig.hoverText} transition-colors`}
        >
          {value}
        </h3>
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md border ${styles[status]} font-bold text-xs uppercase tracking-wider shadow-sm`}
    >
      {icons[status]}
      {status}
    </div>
  ...
  <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-opacity duration-300">
    <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-2xl shadow-2xl animate-[float_0.4s_ease-out] flex flex-col border border-white/50 relative">
      ...
      <div className="px-8 py-6 flex justify-between items-center border-b border-stone-100 bg-stone-50/50">
        <h2 className="text-3xl font-sacred font-bold text-stone-800">
          {title}
        </h2>
        <button
          onClick={onClose}
...
const GeometricInput = ({ label, ...props }: any) => (
  <div className="mb-1 group">
    <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2 ml-1 transition-colors group-focus-within:text-orange-600">
      {label}
    </label>
    <input
      {...props}
...
const AddPriestModal: React.FC<any> = ({ onClose, onAdd }) => {
  ...
  const sub = (e: any) => {
    ...
    onAdd({
      ...f,
      specialization: f.specialization.split(","),
      certification: f.certification.split(","),
      status: "pending",
      ...
      <form onSubmit={sub} className="space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <GeometricInput
            ...
            required
            value={f.priestName}
            onChange={(e: any) => setF({ ...f, priestName: e.target.value })}
          ...
          <GeometricInput
            ...
            required
            value={f.priestPhone}
            onChange={(e: any) => setF({ ...f, priestPhone: e.target.value })}
            placeholder="+91..."
          />
          <GeometricInput
            label="Email"
            ...
            type="email"
            value={f.priestEmail}
            onChange={(e: any) => setF({ ...f, priestEmail: e.target.value })}
          />
          <GeometricInput
            label="City/Location"
            required
            value={f.location}
            onChange={(e: any) => setF({ ...f, location: e.target.value })}
          />
        </div>
        <div className="grid grid-cols-2 gap-6">
          <GeometricInput
            ...
            type="number"
            value={f.experience}
            onChange={(e: any) => setF({ ...f, experience: +e.target.value })}
          />
          <GeometricInput
            label="Education"
            value={f.education}
            onChange={(e: any) => setF({ ...f, education: e.target.value })}
          />
        </div>
        <GeometricInput
          label="Specialization (comma sep)"
          value={f.specialization}
          onChange={(e: any) => setF({ ...f, specialization: e.target.value })}
        />
        ...
        <div className="flex gap-4 pt-6 mt-2 border-t border-stone-100">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-3.5 rounded-lg border-2 border-stone-200 font-bold text-stone-500 hover:bg-stone-50 hover:text-stone-700 hover:border-stone-300 transition-all"
          >
            CANCEL
          </button>
          <button
            type="submit"
            className="flex-1 py-3.5 rounded-lg bg-gradient-to-r from-orange-600 to-red-600 text-white font-bold hover:shadow-lg hover:shadow-orange-200 hover:-translate-y-0.5 transition-all"
          >
            REGISTER PRIEST
          </button>
        </div>
...
const ViewEditPriestModal: React.FC<any> = ({
  priest,
  isEditMode,
  onClose,
  onEdit,
  ...
  return (
    <ModalFrame
      title={isEditMode ? "Edit Details" : "Priest Profile"}
      onClose={onClose}
    >
      <form onSubmit={sub} className="space-y-6">
        {!isEditMode && (
          <div className="flex items-center gap-6 mb-8 pb-8 border-b border-stone-100">
            ...
            <div>
              {/* FIXED: Accessed priest_name instead of PriestName */}
              <h2 className="text-3xl font-bold font-sacred text-stone-800">
                {priest.priest_name}
              </h2>
              <div className="mt-3">
                <StatusBadge status={priest.status || "pending"} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <GeometricInput
            ...
            onChange={(e: any) =>
              setF({ ...f, priest_contact_no: e.target.value })
            }
          />
          <GeometricInput
            disabled={!isEditMode}
          ...
          <GeometricInput
            ...
            onChange={(e: any) =>
              setF({ ...f, priest_present_city_town_village: e.target.value })
            }
          />
          ...
          {isEditMode ? (
            <>
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-3.5 rounded-lg border-2 border-stone-200 font-bold text-stone-500 hover:bg-stone-50 transition-all"
              >
                CANCEL
              </button>
              <button
                type="submit"
                className="flex-1 py-3.5 rounded-lg bg-orange-600 text-white font-bold hover:shadow-lg hover:shadow-orange-200 hover:-translate-y-0.5 transition-all"
              >
                SAVE CHANGES
              </button>
            </>
            ...
            <>
              ...
              <button
                type="button"
                onClick={onDelete}
                className="px-8 py-3.5 rounded-lg bg-red-50 text-red-600 font-bold border border-red-100 hover:bg-red-100 hover:border-red-200 transition-all"
              >
                DELETE
              </button>
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-3.5 rounded-lg bg-stone-800 text-white font-bold hover:bg-stone-900 transition-all shadow-lg shadow-stone-200"
              >
                CLOSE
              </button>
            </>
...
const StatusBadgeCompact = ({ status }: { status: string }) => {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md border ${styles[status]} font-bold text-[10px] uppercase tracking-wider`}
    >
      {icons[status]}
      {status}
    </div>
    ...
    <div className="relative flex justify-end">
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute right-0 top-full mt-1 w-36 bg-white rounded-lg shadow-xl border border-stone-200 py-1 z-[60] overflow-hidden">
            <button
              onClick={() => {
                onView();
                setIsOpen(false);
